...
Эндпойнты:
```
GET / - Используется для сервировки html страницы.

POST /meow - используется для мяукания.
В json запрос задается единственное поле url, ссылка для замяукивания.
В ответе два поля: status (статус запроса) и alias (мяукания).

GET /{alias} - редирект по мяуканиям.
```
